...
public interface RandomPickMapper {
    ...
    @Delete("DELETE FROM class_random_pick WHERE rpick_id = #{rpickId}")
    int deleteRandomPickById(Long rpickId);

    @Update("UPDATE class_random_pick SET remark = #{remark} WHERE rpick_id = #{rpickId}")
    int updateRandomPick(RandomPickRecord record);
}
