...
public interface ParticipationHeatMapper {
    ...
    List<Map<String, Object>> selectParticipationHeatDataByCondition(@Param("studentNo") String studentNo,
    ...
    @Select("SELECT " +
            ...
            "    SUM(CASE WHEN participation_score >= 90 THEN 1 ELSE 0 END) as level_high_count, " +
            "    SUM(CASE WHEN participation_score >= 80 AND participation_score < 90 THEN 1 ELSE 0 END) as level_good_count, " +
            "    SUM(CASE WHEN participation_score >= 70 AND participation_score < 80 THEN 1 ELSE 0 END) as level_medium_count, " +
            "    SUM(CASE WHEN participation_score >= 60 AND participation_score < 70 THEN 1 ELSE 0 END) as level_normal_count, " +
            "    SUM(CASE WHEN participation_score < 60 THEN 1 ELSE 0 END) as level_low_count " +
            "FROM ( " +
            "    SELECT " +
            "        s.student_id, " +
            "        COALESCE(ROUND( " +
            ...
            "            (SELECT COUNT(*) FROM class_forum_comment cfc WHERE cfc.user_id = s.student_id AND cfc.del_flag = '0') * 2 + " +
            "            (SELECT COUNT(*) FROM class_forum_like cfl WHERE cfl.user_id = s.student_id AND cfl.del_flag = '0') " +
            "        , 0) * 0.3 as participation_score " +
            "    FROM class_student s " +
            "    WHERE s.status = 1 " +
            "    AND (#{studentNo} IS NULL OR s.student_no LIKE CONCAT('%', #{studentNo}, '%')) " +
            "    AND (#{studentName} IS NULL OR s.student_name LIKE CONCAT('%', #{studentName}, '%')) " +
            ") t")
    Map<String, Object> selectParticipationDistributionByFilter(@Param("studentNo") String studentNo,
                                                                @Param("studentName") String studentName);
}